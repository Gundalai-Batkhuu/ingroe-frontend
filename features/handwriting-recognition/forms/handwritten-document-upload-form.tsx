...
const HandwrittenDocumentUploadForm = ({
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (response && response.file_id) {
				...
				try {
					const text = await getTextFromS3(
						response.file_map.file_url
					);
					console.log(
						'Retrieved text:',
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			{message && (
				<div
					className={`mt-4 rounded p-2 ${message.type === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
				>
