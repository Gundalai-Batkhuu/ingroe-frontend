...
export function SharedArtifactsTable({ userId, searchParams }: SharedArtifactsTableProps) {
  ...
  if (!artifacts || !artifacts.shared_documents_owned) return <div>No shared artifacts found</div>

  const filteredDocuments = artifacts.shared_documents_owned.filter(doc =>
    artifacts.artefact_tree.find(art => art.document_id === doc.document_id)?.document_name.toLowerCase().includes(search.toLowerCase())
  )
  ...
  function prevPage() {
    router.push(`/shared?offset=${Math.max(0, offset - 5)}`, { scroll: false })
  }
            ...
            {paginatedDocuments.map((doc: SharedDocument) => {
              ...
              return (
                <TableRow key={doc.document_id}>
                  <TableCell>{artifact?.document_name || 'Unknown'}</TableCell>
                  <TableCell>{new Date(doc.shared_at).toLocaleString()}</TableCell>
                  <TableCell>{doc.public_access ? 'Yes' : 'No'}</TableCell>
                  <TableCell>{doc.access_open ? 'Yes' : 'No'}</TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onSelect={() => handleRevokeAccess(doc.document_id)}>
                          Revoke Access
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
