...
export function DesktopNavbar() {
	...
	return (
		<aside
			className={`border-r bg-background duration-300 ease-in-out ${isNavbarExpanded ? 'w-56' : 'w-14'}`}
		>
			<nav className="flex h-screen flex-col items-center gap-4 overflow-hidden px-2">
				<div className="relative ml-4 mt-2 flex h-14 w-full items-center justify-start gap-2">
					<AppLogo className={`${!isNavbarExpanded && 'hidden'}`} />
					<span
						className={`text-2xl font-semibold text-brand-green ${!isNavbarExpanded && 'hidden'}`}
					>
						Ingroe
					</span>
					<Button
				...
				/>
				<div className="mb-4 mt-auto flex w-full flex-col gap-4">
					<HelpCenter
						isNavbarExpanded={isNavbarExpanded}
						toggleNavbar={toggleNavbar}
					/>
