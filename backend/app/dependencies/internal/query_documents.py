...
class QueryDocument:
    ...
    def _get_entity_extraction_prompt(cls) -> ChatPromptTemplate:
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are extracting all the entities from the text such as all the person, organization, streets and entities related to geography, finance, time and dates, legal information, products or services, medical information, events, cultural and media references, technical information and miscellaneous information that appear in the text"
                ),
                (
                    "human",
    ...
    def _structured_retriever(cls, question: str, parent_id: str) -> str:
        """
        Returns a entity relationship string with each relationship between entities spearated by a new line. It checks for bidirectional relationship as show by UNION ALL.
        """
    ...
    @classmethod
    def _get_condensed_prompt(cls) -> str:
        """
    ...
    @classmethod
    def _get_final_prompt() -> ChatPromptTemplate:
    ...
    @classmethod
    def _get_chain(cls) -> Any:
        search_query = cls._get_search_query()
        chain = (
        RunnableParallel(
            {
    ...
    @classmethod
    def query_document(cls, query: str, parent_id: str):
        chain = cls._get_chain()
        chain.invoke({"question": query, "parent_id": parent_id})
        print(query)